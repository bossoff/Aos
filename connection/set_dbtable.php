...
if($con == false){
	...
	if(mysqli_query($con,$DATABASE)){
		...
		if($dbrr == "The DATABASE as been successful Created."){
			$server = 'localhost'; $db_name = 'root'; $db_password = ''; $db = 'ngmedia';
			$connect = mysqli_connect($server, $db_name, $db_password) or die("Sorry Could not to the Server Sir".mysqli_error($con));
