...
if(isset($_POST['login']) && $_POST['login']=='ogins'){
	...
	if(empty($email) || empty($password)){
		...
		if($email != $get_admin_er['email'] && $password != $get_admin_er['password']){
			$er = true;
			$_SESSION['ermsg'] = "Invalid to login cridentials";
		}
		...
		elseif($query_admin_num==0){
			// $password = md5(sha1($password)).sha1($password);
			$query_users = mysqli_query($connect, "SELECT email, application_no, password FROM users WHERE password = '$password' AND email = '$email' || application_no = '$email' LIMIT 1");
			 $get = mysqli_fetch_assoc($query_users);
				if($email != $get['email'] || $email != $get['application_no'] && $password != $get['password']){
					$er = true;
					$_SESSION['ermsg'] = "Invalid to login Email and Password cridentials";
				}
			...
			if($get['password'] == $password){
				...
				if(!empty($check_users)){
					$data = mysqli_fetch_assoc($check_users);
						...
						echo $us_password = $_SESSION['us_password'] = $data['password'];
