...
if(isset($_POST['Profile']) && $_POST['Profile'] == 'Updateed'){
    ...
    // $email = mysqli_real_escape_string($connect, htmlspecialchars(htmlentities(sanitize(clean(strtolower($_POST['email']))))));
    $surname = mysqli_real_escape_string($connect, htmlspecialchars(htmlentities(sanitize(clean(ucwords(strtolower(removeBadChars($_POST['surname']))))))));
    ...
    $middle = mysqli_real_escape_string($connect, htmlspecialchars(htmlentities(sanitize(clean(ucwords(strtolower(removeBadChars($_POST['middle']))))))));
        ...
        $image_allowed_size = 1024;//kb
         $Representgallery_userfile = strtoupper($surname.'_'.$uid."_profile")."_".date("y").".".$image_extension;
      ...
      if($er == false){
        if($image_type=='image/jpeg' || $image_type=='image/png' || $image_type=='image/jpg' || $image_type=='image/gif'){  
             $query_update = mysqli_query($connect, "UPDATE users SET firstname = '$lastname', email = '$email', lastname = '$middle', surname = '$surname',default_avartar = 1, avartar = 'upload/$Representgallery_userfile', fullname = '$fullname' WHERE user_id = '$uid' AND user_level = '$user_level'")or die();
...
if(isset($_POST['change']) && $_POST['change'] == 'pass'){
    $er = false;
    $password = $_POST['cpassword'];//mysqli_real_escape_string($connect, htmlspecialchars(htmlentities(sanitize(clean(ucwords(strtolower(removeBadChars($_POST['cpassword']))))))));
    ...
    $query_password = mysqli_query($connect, "SELECT password FROM users WHERE user_id = '$uid' AND user_level = '$user_level'");
...
// echo $password;
    if($new_password!=$confirm_new_password){
        ...
        if($er==false){
            ...
            $insert = mysqli_query($connect, "UPDATE users SET password = '$new_password' WHERE user_id = '$uid' AND user_level = '$user_level'");
            if(!empty($insert)){
                $er = false;
                    ...
                    <div class="box-content">
                           <form action="<?=base_url('manage-profile.php?');?>" method="POST" class="form-horizontal form-groups-bordered validate" enctype="multipart/form-data"> 
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-5">
                                        <input type="text" class="form-control" name="name" readonly value="<?php echo $get_data['fullname'];?>"/>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-5">
                                        <input type="email" class="form-control" name="email" value="<?php echo $get_data['email'];?>"/>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-5">
                                        <input type="text" class="form-control" name="surname" value="<?php echo $get_data['surname'];?>"/>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-5">
                                        <input type="text" class="form-control" name="lastname" value="<?php echo $get_data['firstname'];?>"/>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-5">
                                        <input type="text" class="form-control" name="middle" value="<?php echo $get_data['lastname'];?>"/>
                                    </div>
                                </div>
