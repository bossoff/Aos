...
function check_login(){
  if(strlen($_SESSION['uid'] && $_SESSION['user_level'])==0 || $_SESSION['us_password'] == 0){
    header("Location:" .base_home()."?rdir=no_entry_session_not_set");
        exit();
  }

  elseif(!isset($_SESSION['uid']) || !isset($_SESSION['user_level']) || !isset($_SESSION['us_password'])){  
         header("Location:" .base_home("").'?rdir=warning_is_not_session!');
          exit();
       
  }

  elseif(isset($_SESSION['uid']) && isset($_SESSION['us_password']) && isset($_SESSION['user_level'])){
        $user_level = preg_replace('#[^a-zA-Z]#i', '', $_SESSION['user_level']);
        $user_level = preg_replace('#[^A-Za-z0-9]#i', '', $_SESSION['user_level']);
        $user_level = preg_match("/^[a-zA-Z ]*$/", $_SESSION['user_level']);
  ...
  $check_user_information = mysqli_query($connect, sprintf("SELECT user_id, user_level FROM users WHERE password = '".$_SESSION['us_password']."' AND user_id = '".$_SESSION['uid']."' AND user_level = '".$_SESSION['user_level']."'"));
