...
<title><?=TITLE7;?></title>
		...
		<table class="table table-bordered datatable" id="table-4">
			...
			$Query_french = mysqli_query($connect, "SELECT * FROM users WHERE user_level = 'French'");
			 while($get_french_department = mysqli_fetch_assoc($Query_french)){
					$Surname = $get_french_department['surname'];
					$fullname = $get_french_department['fullname'];
					$phone = $get_french_department['phone'];
					$email = $get_french_department['email'];
					$application = $get_french_department['application_no'];
				?>
				...
				<tr class="odd gradeX">
					...
					<td><?= $Surname;?></td>
					<td><?= $fullname;?></td>
					<td class="warning"><?= $application;?></td>
