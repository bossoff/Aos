...
<title><?=TITLE7;?></title>
		...
		<table class="table table-bordered datatable" id="table-4">
			...
			$Query_project = mysqli_query($connect, "SELECT * FROM users WHERE user_level = 'Project'");
			 while($get_project_department = mysqli_fetch_assoc($Query_project)){
					$Surname = $get_project_department['surname'];
					$fullname = $get_project_department['fullname'];
					$phone = $get_project_department['phone'];
					$email = $get_project_department['email'];
					$application = $get_project_department['application_no'];
