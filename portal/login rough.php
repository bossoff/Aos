				...
				if($users_num>0){
					...
					$user_level = $get_user['user_level'];	
						...
						if($check_num == 0){							
							...
							$logCONT1 = mysqli_query($connect, "UPDATE memberslog SET loginCount = loginCount+1, userip = '$ip_address',status = '$status', loginTime = '$currentTime' WHERE email = '$email' AND uid = '$uid'")or die(mysqli_error($connect));
							if(!empty($logCONT1)){
								header("LOCATION:".base_url("section/dp/home.php?".$acc."_rdir=Welcom&&Successful_'".base64_encode(active)."'Active_into/AOS=1_".$acc));
							}
						}
