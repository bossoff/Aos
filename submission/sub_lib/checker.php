<?php
  ...
  //CHINKING IF WHO IS TRYING TO THIS PAGE IS THE RIGHT PERSON
   if (!isset($_SESSION['userid']) || !isset($_SESSION['user_login']) || !isset($_SESSION['user_email']) || !isset($_SESSION['phone'])) {
      ...
      if(session_destroy()){
       header("Location:" .base_url("html/session_expired.html?rdir=No_access"));
        exit;
      } 
  }
 ...
 // include "dbconnect/database.php";
  ...
  //      $username = $row['user_login'];
   ...
   $check_username = sprintf("SELECT user_login FROM user WHERE user_login = '$user_login'|| user_email = '$user_email' || phone = '$phone' AND user_password= '$user_password' LIMIT 1");
