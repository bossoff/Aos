<?php
session_start();
if($protocol = $_SERVER['REQUEST_SCHEME']){

	if($protocol == 'http'){
		$urk= "http://$_SERVER[HTTP_HOST]";	
		function base_url($value){
			$urk= "http://$_SERVER[HTTP_HOST]";	
			$url = $urk.'/aos/blog/'.$value;
			return $url;	
		}
...
function clean_fun($value){
	// $bad = htmlentities(trim($value))
	$bad = removeBadChars(sanitize(clean(htmlentities(trim($value)))));
	return $bad;
}
