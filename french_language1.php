...
if(isset($_POST['create']) && $_POST['create'] == "Register" && $_POST['create'] !=""){
	...
	$email = $_SESSION['email'] = mysqli_real_escape_string($connect, htmlspecialchars(htmlentities(sanitize(clean(strtolower(removeBadChars($_POST['email'])))))));
	...
	$adult_education = mysqli_real_escape_string($connect, $_POST['adult_education']);
	...
	if($num3>0){
		...
		if($er == false){			
			...
			if($application_no_result==0){
					$application_no = $_SESSION['application_no'] = 'AOS'."/".date("y")."/".strtoupper("FLS"."/".'01');
				...
				$query_insert = mysqli_query($connect, sprintf("INSERT INTO users SET surname = '$surname', user_level = '$user_level', email = '$email', application_no = '$application_no', gender = '$gender', dob = '$dob', phone = '$phone', program = '$program', class = '$class', adult_education = '$adult_education', ip_address = '$ip_address', department = 'French Studies', password = '$password', creation_date = '$currentTime', session ='".date("Y")."'")) or die(mysqli_error($connect));
				...
				if(!empty($query_insert)){
					...
					$mail = Send_Mail($email,$surname,$subject,$body,$altbody);
							...
							header("LOCATION:".base_url("cridential.php?".$send."_rdir=otp&access=".$active."/Active_into/AOS_French%Language%Studies_log=1_".$number));
							...
							//  $_SESSION['application_no'];
							//  $_SESSION['echomsg'] = ' Your French Laguage Department account as been Created.';
							// header("LOCATION:".base_url("cridential.php?".$send."_rdir=otp&access=".$active."/Active_into/AOS_French%Language%Studies_log=1_".$phone));
				...
				$application_no = $_SESSION['application_no'] = 'AOS'."/".date("y")."/".strtoupper("FLS"."/".$nextneededid);
				// insert emails to the email list 
					$check_mails = mysqli_query($connect, "SELECT email FROM emails") or die(mysqli_error($connect));
					$getmails = mysqli_fetch_assoc($check_mails);
					if($getmails['email']==$email){
																				...
																				<div class="contentTextEditable">
																                	<div class="contentEditable" align="left" >
																                  		<p style="color:#555;font-family:Helvetica, Arial, sans-serif;font-size:16px;line-height:160%;"><strong>Hi '.$surname.' </strong> 
																							...
																							You have been succesfull registered for <a href="http://aosacademy.com">A.O.S Academy (FRENCH LANGUAGE STUDIES)</a>. Here are your Login Cridentials <br/> Email: "'.$email.'"<br/>Username:"'.$application_no.'".<br/>password:"'.$surname.'".<br/>We appreciate your interest in A.O.S Academy services. We are the best as a topflight professionals in Fench Language Studies.<br/>For more enquries contact us on: "'.MTN.','.AIRTEL.'
																						</p>
																                	</div>
																              	</div>
																			</td>
...
<title><?=TITLE13;?></title>
				...
				<div class="row">
					<div class="col-lg-12">
						<ul class="breadcrumb">
							...
							<li class="active">Create French Acount</li>
						</ul>
					</div>
				</div>
			</div>
	...
	<div class="row">
					<div class="col-xs-12 col-sm-8 col-md-6 col-sm-offset-2 col-md-offset-3">
						<form role="form" action="<?=base_url("french_language.php?rdir=set_account&&account==create_account/input.php");?>" method="POST" class="register-form" enctype="multipart/form-data">
							...
							<div class="row">
								<div class="col-xs-12 col-sm-6 col-md-6">
									...
									<div class="form-group">
										<input type="text" name="dob" id="dob" title="Date Of Birth" value="" class="form-control input-lg" required="required" placeholder="29/2/1991" tabindex="6">
									</div>
								</div>
							...
							<div class="form-group">
								<div style="background: green; padding: 5px; color: #fff; text-align: center;">APPLICATIONS DETAILS </div>
							</div>

								<div class="col-xs-12 col-sm-6 col-md-6">
										<div class="form-group">
											<select name="program" class="form-control input-lg" onChange="getlevel(this.value);" required="required">
												<option value="" required>-- Select Program --</option>

												<?php $ret=mysqli_query($connect,"SELECT * FROM application_details LIMIT 3");
												while($row=mysqli_fetch_assoc($ret)){
												?>

													<option value="<?php echo htmlentities($row['program']);?>">
								...
								<div class="col-xs-12 col-sm-6 col-md-6">
									<div class="form-group">
										<select name="class" class="form-control input-lg" onChange="getlevel(this.value);" required="required">
											<option value="" required>-- Select Class --</option>

											<?php $ret=mysqli_query($connect,"SELECT * FROM application_details LIMIT 2");
											while($row=mysqli_fetch_assoc($ret)){
											?>
							...
							<div class="form-group">
									<select name="adult_education" class="form-control input-lg" onChange="getlevel(this.value);">
												<option value="">-- If Adult Education --</option>

												<?php $ret=mysqli_query($connect,"SELECT * FROM application_details LIMIT 2");
												while($row=mysqli_fetch_assoc($ret)){
												?>

													<option value="<?php echo htmlentities($row['adult_education']);?>">
														<?php echo htmlentities($row['adult_education']);?>
													</option>
													<?php } ?>
							...
							<div class="row">
								...
								<div class="col-xs-12 col-md-6">Already have an account? <a href="<?=portal("");?>">Sign In</a></div>
							</div>
